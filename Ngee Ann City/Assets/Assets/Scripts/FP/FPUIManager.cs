...
{
    public static FPUIManager Instance { get; private set; }
    ...
    public FPGridManager fpGrid; // Reference to the GridManager
    ...
    public void OnBuildClicked()
    {
        if (int.TryParse(xInput.text, out int x) && int.TryParse(yInput.text, out int y))
        {
            ...
            {
                if (fpGrid != null)
                    fpGrid.Build(x, y, selectedBuildingIndex);
                else
    ...
    public void OnDestroyClicked()
    {
        if (int.TryParse(xInput.text, out int x) && int.TryParse(yInput.text, out int y))
        {
            FPGridManager.Instance.DestroyBuilding(x, y);
        }
        else
        {
