...
public class FPGameManager : MonoBehaviour
{
    ...
    private void ApplyLoadedGameData(SaveData saveData)
    {
        ...
        // Set the grid size before generating the grid
        griddy.SetGridSize(saveData.gridWidth, saveData.gridHeight);

        // Generate the grid with the new size
        griddy.GenerateGrid();
