...
public class LoadingUIManager : MonoBehaviour
{
    ...
    public GridManagerForLoading griddy; // Reference to the GridManager
    ...
    public void OnBuildClicked()
    {
        if (int.TryParse(xInput.text, out int x) && int.TryParse(yInput.text, out int y))
        {
            ...
            {
                if (griddy != null)
                    griddy.Build(x, y, selectedBuildingIndex);
                else
    ...
    public void OnDestroyClicked()
    {
        if (int.TryParse(xInput.text, out int x) && int.TryParse(yInput.text, out int y))
        {
            GridManagerForLoading.Instance.DestroyBuilding(x, y);
        }
        else
        {
